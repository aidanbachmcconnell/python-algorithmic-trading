# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 13:30:10 2019
...
def jpy_trade(sl,tp,pair,current_price):
    ...
    units=str(units_calc(bal,.01,sl))
    stoploss=str(stoploss_calc(pair,wick,sl))
    takeprofit=str(takeprofit_calc(pair,current_price,tp))
    ...
    api_order=orders.OrderCreate(accountID,data=order)
    api.request(api_order)
