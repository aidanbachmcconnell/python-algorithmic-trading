# -*- coding: utf-8 -*-
"""
Created on Fri Apr 26 11:34:25 2019
...
x=pd.read_csv('C://Users/U698198/Documents/USD_CHF.csv')
df=x[~(x==0).any(axis=1)]
y=x
...
print('pd_forex_data',pd_forex_data.head())            
...
#print(np_final.shape,dz.head())
drop_zeros.head()
cols=drop_zeros.columns.tolist()
cols=['Open',
 'High',
 'Low',
 'Close',
 ...
 'last_16','Local_Time']
cols
drop_zeros=drop_zeros[cols]
drop_zeros.head()
dz=dz=drop_zeros.iloc[:,[0,1,2,3,7,10,11,12,13,14,15,16,17,18]]

np_final=dz.values
print('what the algo needs: ',dz.head())
...
def Run_hr(forex):
    ...
    for i in range(data.shape[0]):
        for j in range (1,30):
            ...
            Support(data,i,j,.001,.0025,result_array)
            Resistance(data,i,j,.001,.0025,result_array)#0.04 0.2
    return result_array

aidan=Run_hr(np_final)
ss=time.time()
...
print('R:R',a[3]/-a[4])
print()
TE=((a[0]/(a[0]+a[1]))*.01*a[3]/-a[4])-((1-(a[0]/(a[0]+a[1])))*.01)
