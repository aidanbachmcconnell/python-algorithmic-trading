# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 19:52:51 2019
...
for pair in pairs:    
    ...
    def Run_hr(forex):
        ...
        for i in range(data.shape[0]):
            for j in range (1,30):
                if "JPY" in pair:
                    ResistanceatWick(data,i,j,.1,.2,result_array)#0.12 0.21 TE=1.279% for 2% risk
                    SupportatWick(data,i,j,.1,.2,result_array) #0.11 0.22 TE=1.45% for 2% risk
                else:
                    ResistanceatWick(data,i,j,.001,.002,result_array)#0.12 0.21 TE=1.279% for 2% risk
                    SupportatWick(data,i,j,.001,.002,result_array)
